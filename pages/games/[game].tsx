...
export default function GameInspect() {
  ...
  const [gameObject, setGameObject] = useState<IGame | undefined>(undefined);
  const [pageState, setPageState] = useState<"loading" | "done">("loading");
  ...
  useEffect(() => {
    if (router.isReady) {
      const gameObject = getGameById(router.query.game as string);
      if (gameObject.id === "nogame") {
        router.push("/gamenotfound");
      } else {
  ...
  if (router.isFallback) {
    // TODO: Appropriate content loading page / component
